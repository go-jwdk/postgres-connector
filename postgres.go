...
func (sqlTemplate) NewFindJobsDML(table string, limit int64) (stmt string, args []interface{}) {
	query := `
SELECT * FROM %s WHERE invisible_until <= extract(epoch from now()) ORDER BY sec_id DESC LIMIT %d
`
...
func (sqlTemplate) NewUpdateJobByVisibilityTimeoutDML(queueRawName string, jobID string, visibilityTimeout int64) (stmt string, args []interface{}) {
	query := `
UPDATE %s SET invisible_until = extract(epoch from now()) + ? WHERE job_id = ?
`
...
CREATE TABLE IF NOT EXISTS %s_queue_attributes (
		...
		visibility_timeout       INTEGER NOT NULL DEFAULT,
		delay_seconds            INTEGER NOT NULL DEFAULT,
		max_receive_count        INTEGER NOT NULL DEFAULT,
